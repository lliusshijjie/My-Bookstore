...
inline HttpResponse handle_add_inventory(const HttpRequest& request, InventoryClient& client)
{
    auto it = request.path_params.find("book_id");
    if (it == request.path_params.end()) {
        return HttpResponse::json(400, "{\"code\":400,\"message\":\"missing book_id\",\"data\":null}");
    ...
    int book_id = std::atoi(it->second.c_str());
    auto quantity = json_int_value(request.body, "quantity");
    if (!quantity.has_value() || *quantity <= 0) {
        return HttpResponse::json(400, "{\"code\":400,\"message\":\"invalid inbound request\",\"data\":null}");
    }

    auto stock = client.add_stock(book_id, *quantity);
    if (!stock.has_value()) {
        return HttpResponse::json(404, "{\"code\":404,\"message\":\"inventory not found\",\"data\":null}");
    }
